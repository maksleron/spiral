...
def is_number(d):
    #Проверка что d это цифровое значение
    p = re.compile(r"\A[-]?\d+(?:\.\d+)?\Z")
    m = p.search(d)
    return bool(m)

def is_operator(o):
    #Проверка что o является арифметическим оператором
    p = re.compile(r'\A[\+\-\*\/\%]\Z')
    m = p.search(o)
    return bool(m)
...
class Interpreter:
    ...
    def precedence(self, operator):
        if operator == '+' or operator == '-':
            ...
            raise Exception("%s не является арифметическим оператором." % operator)
    ...
    def shunting_yard(self, expression):
        ...
        for token in expression:
            if is_number(token):
                if last == "number":
                    raise Exception("ОШИБКА: Неверный синтаксис: %r" % expression)
                    return
                last = "number"
                try:
                ...
                try:
                    ...
                    raise Exception("ОШИБКА: Некорретная расстановка скобок!")
                    return
            else:
                raise Exception("ОШИБКА: Неподдерживаемый символ: %r" % token)
                return
        while operators:
            output.append(self.operators[operators.pop()])
        return output
...
output = interpreter.input("(11+12)*13/14")
print(output)
output = interpreter.input("(11+12)*13/14)")
print(output)